...
namespace bartender_control 
{
    ...
    void OneTaskInverseKinematics::command(const bartender_control::bartender_msg::ConstPtr &msg)
    {
        ...
        alpha1 = msg->position_task_gain;
        alpha2 = msg->gravity_task_gain;

        second_task = msg->multi_task;

        if (msg->do_rot_z) rot_z = msg->rot_z;
        if (!msg->do_rot_z) rot_z = 0;

        do_rot_Z = msg->do_rot_z;
    }
    ...
    void OneTaskInverseKinematics::param_update()
    {
        ...
        {
            // x_.M.DoRotZ(rot_z);
            x_des_.M.DoRotZ(rot_z);
            do_rot_Z = false;
        }

        /*x_.M.EulerZYZ(rot_z, 0, 0);
        x_des_.M.EulerZYZ(rot_z, 0, 0);*/
    ...
    void OneTaskInverseKinematics::update(const ros::Time& time, const ros::Duration& period)
    {
        ...
        // bartender_control::bartender_msg msg_error;
        ...
        if (cmd_flag_)
        {
            ...
            /*msg_error.data.push_back( x_err_.p(0) );
            msg_error.data.push_back( x_err_.p(1) );
            msg_error.data.push_back( x_err_.p(2) );*/

            x_.M.GetRPY(x_or[0],x_or[1],x_or[2]);   //.M is referref to the rotation of x_. The function GetQuaternion obtains quaternion from the rotation matrix
            x_des_.M.GetRPY(x_des_or[0],x_des_or[1],x_des_or[2]);

            msg_error.data.push_back( x_des_or[0] - x_or[0] );
            msg_error.data.push_back( x_des_or[1] - x_or[1] );
            msg_error.data.push_back( x_des_or[2] - x_or[2] );    

            // msg_error.des_frame = x_diff_;
